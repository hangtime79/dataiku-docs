...
def setup(app: Sphinx):
    ...
    app.add_config_value("enable_on_the_same_page", True, "html")
    ...
    app.add_config_value(
        "tippy_js",
        ("https://cdnjs.cloudflare.com/ajax/libs/popper.js/2.11.8/umd/popper.min.js", "https://cdnjs.cloudflare.com/ajax/libs/tippy.js/6.3.7/tippy-bundle.umd.min.js"),
        "html",
        [list, tuple],
    )
...
def compile_config(app: Sphinx):
    ...
    if "delay" in props:
        if not (props["delay"] is None or isinstance(props["delay"], str)):
    ...
    app.env.tippy_config = TippyConfig(  # type: ignore[attr-defined]
        ...
        enable_on_the_same_page=app.config.enable_on_the_same_page,
...
def collect_tips(
    ...
    for anchor in body.find_all("a", {"href": True}):
        ...
        if tippy_config.enable_on_the_same_page and not path:
