...
- By default, all rows are used to produce the chart. To focus the chart on a subset of records, drag one or more columns to the **Filters** control in the **Setup** tab and select which values of those columns should be displayed, and which should be filtered out. For more information about filter settings, see :doc:`filter-settings`.

- **Color** controls in the **Format** tab define the color in the charts.  See :doc:`palettes` for details.
