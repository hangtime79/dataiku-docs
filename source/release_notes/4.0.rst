...
----------------------------
	...
	* From DSS 3.1: Automatic migration is supported, with the restrictions and warnings described in :ref:`releases_notes.4.0.limitations`

	* From DSS 3.0: In addition to the restrictions and warnings described in :ref:`releases_notes.4.0.limitations`, you need to pay attention to the restrictions and warnings applying to DSS 3.0. See :doc:`3.0 -> 3.1 </release_notes/3.1>`
	...
	* From DSS 2.X: In addition to the following restrictions and warnings, you need to pay attention
	  ...
	  :doc:`2.3 -> 3.0 </release_notes/3.0>` and :doc:`3.0 -> 3.1 </release_notes/3.1>`
...
Pay attention to the warnings described in :ref:`releases_notes.4.0.limitations`.


.. _releases_notes.4.0.limitations:
...
In Hadoop settings, previous versions of DSS didn't use the HiveServer2 component. DSS now uses and requires HiveServer2 for all interaction with Hive. HiveServer2 is included by default in all Hadoop distributions. See :doc:`/hadoop/hive` for more information.
...
Since the addition of the new dashboard, :doc:`Webapps </webapps/index>` have been moved to their own section in the UI. You'll find the usual webapp editor in the "Notebooks" section of the project ("Web Apps" subtab)

For webapps that have a Python backend, make sure that the python backend file does not contain
`encoding magic comment <https://www.python.org/dev/peps/pep-0263/>`_, such as:
...
* Models trained with prior versions of DSS must be retrained when upgrading to 4.0 (usual limitations on retraining models and regenerating API node packages - see :doc:`/installation/custom/upgrade`). This includes models deployed to the flow (re-run the training recipe), models in analysis (retrain them before deploying) and API package models (retrain the flow saved model and build a new package)
...
* **New feature**: HDFS connections can now reference any kind of HDFS URL, not only paths on the default FS. :doc:`This makes it possible to read s3://, s3a://, wasb://, adl:// and others through HDFS connections </hadoop/hadoop-fs-connections>`. Credentials are passed through additional per-connection properties (*Limitation: using S3 as a HDFS filesystem is not supported on MapR*)
...
For more information, see :doc:`/dashboards/index`.
...
For more information, see :doc:`/user-isolation/index`
...
For more information, see :doc:`/spark/index`
...
For more information, see :doc:`/spark/pipelines`
...
For more information, see :doc:`/spark/usage`
...
The "Analyze" view in Explore can now be based on the whole dataset (in addition to the exploration sample). This is available on all dataset types and will automatically run in database, Hive or Impala depending on the type of dataset and available engines. See :doc:`/explore/analyze` for more information.
...
More information is available at :doc:`/operations/macros`
...
* :doc:`Sampling in explore and visual data prep </explore/sampling>`
* :doc:`Sampling in other parts of DSS </sampling/index>`
...
* Advanced options and statements splitting capabilities have been added to the SQL Query recipe. See :doc:`/code_recipes/sql` for more information. This makes it easier to do advanced things like stored procedures or CTEs in SQL recipes.
* SQL script recipes can now automatically infer output schema, like SQL query recipes. See :doc:`/code_recipes/sql` for more information
* SparkSQL recipes can now use the global Hive metastore, alternatively to using only the local datasets. See :doc:`/code_recipes/sparksql` for more information
...
* In addition to multi-user security and audit-trail, the permissions system has been overhauled and new permission definitions have been introduced. You can now define thinner-grained group permissions at the project level. See :doc:`/security/permissions`.
* More options are available for sharing items between projects, and authorizing objects on dashboards. See :doc:`/security/shared-objects`.
...
For more information, see :doc:`/publicapi/index`
...
* New options are available for making datasets "relocatable", easing copying and reimporting projects, while avoiding conflicts between projects. See :doc:`/connecting/relocation`.
