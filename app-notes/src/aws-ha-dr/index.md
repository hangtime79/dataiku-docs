# Howto: High-Availability and Disaster Recovery on AWS

  * [1. Overview](#1-overview)
    + [1.1. Introduction](#11-introduction)
    + [1.2. Not covered](#12-notcovered)
  * [2. Intra-AZ HA with EBS](#2-intra-az-ha)
    + [2.1. Manual failover](#21-intra-az-ha-manual)
    + [2.2. Automated failover](#22-intra-az-ha-auto)
  * [3. Inter-AZ HA](#3-inter-az-ha)
  * [4. Disaster recovery with EBS snapshots](#4-dr-ebs)
    + [4.1. Snapshot strategy](#41-dr-ebs-snapshot)
    + [4.2. Complete restore](#42-dr-ebs-complete)
    + [4.3. Partial restore](#43-dr-ebs-partial)
  * [5. Conclusion and recommended setup](#5-conclusion)
...
This document describe strategies for implementing disaster-recovery and failover / high-availability capabilities for the Dataiku DSS design and automation nodes, when running on AWS.
...
* AWS-specific: EC2, EBS, EBS Snapshots, EFS, AZ
* Generic: RPO, RTO
...
* Hardware failure of the EC2 instance running the DSS instance
* Persistent system-level software failure of the EC2 instance running the DSS instance
* (For some setups) Complete failure of the AWS AZ running the DSS instance
...
* Failure and loss of data of the EBS hosting the DSS data dir
...
# 2. Intra-AZ HA / failover with EBS

When running on a cloud provider, compute and storage are natively separated (between EC2 instance and EBS in the case of AWS). 

Furthermore, apart from an extreme data loss case (which is covered as part of DR, not HA), the storage (EBS) is assumed to be itself highly available.

The basic intra-AZ failover scenario for DSS is therefore based on:

* Shutting down a malfunctioning (either software or hardware) EC2 instance 
* Restarting another EC2 instance which reattaches to the same EBS containing the DSS data directory.
* Moving an IP or DNS entry to the new EC2 instance
...
* Decision criteria to trigger the failover plan. For example, unresponsiveness of the EC2 instance that is not correct after trying to restart the EC2 instance, or AWS indicating a failure of the instance.

* Procedure, which can be either fully manual or scripted, to perform the switch (bringing up the new EC2 instance, attaching the EBS, starting DSS, moving the IP / DNS entry)
...
<a name="3-inter-az-ha"></a>

# 3. Inter-AZ HA

The previous scenario only covers software or hardware failure of the single EC2 node running the DSS software. It does not cover the possibility of partial or complete AZ failure, manifesting by either the unability to create new EC2 instances, or complete unavailability of the EBS service in an availability zone.

AWS EFS (Elastic File System), which provides a regional-scoped filesystem accessible through NFS protocol, could look like a solution to provide HA that resists an AWS AZ failure.

However, **DSS Does not support AWS EFS**. This is due to the fact that EFS is a NFS filesystem, which does not provide fully valid semantics. These semantic issues make EFS not compatible with Dataiku's User Isolation Framework (ACLs are not supported), and have proven to create crashes and data corruptions

Furthermore, EFS has proven to have unacceptable performance for smooth operation of DSS.

You should carefully consider whether you truly require HA (i.e. zero RPO) in face of an AWS AZ failure (which is an extremely rare event - much less than once per year in average). Using only DR in case of an AZ failure can yield low RTO (10-30 minutes), and low RPO (sub-hour RPO is very realistic).


<a name="4-dr-ebs"></a>

# 4. DR with EBS snapshots
...
The recommended way to perform historized backups on AWS is to use EBS snapshots.
...
With this strategy, you are able to maintain a low RPO (one hour) in the case of a disaster that is noticed immediately, while maintaining a granular RPO (hour by hour) for recent disasters, and less granular in case a disaster remains unnoticed for a long time

<a name="42-dr-ebs-complete"></a>

## 4.2 Complete restore strategy

Restoring a complete data directory from a EBS snapshot is fairly similar to the manual failover procedure described above:

* Shut down the EC2 instance running DSS
* Restore the snapshot to a new EBS volume
* Create a new EC2 instance attaching this EBS volume
* Move an IP or DNS entry to this new EC2 instance

<a name="43-dr-ebs-partial"></a>

## 4.3. Partial restore strategy
...
In that case, you can simply mount the EBS snapshot as a new EBS volume on the main EC2 instance running DSS, and copy files over from this restored volume to the DSS data directory.
...
## 5. Conclusion and recommended setup

Overall, given its good complexity / constraints / RPO and RTO characteristics, Dataiku recommends the following setup for HA and DR on AWS:

* Use intra-AZ failover based on EBS reattachment. Have a script or written procedure for failover and perform manual failover in case of failure. Regularly test your failover capability.

* Use DR based on historized EBS snapshots. Have a written procedure for recovery after disaster and perform manual disaster recovery in case of failure. Regularly test your recovery capability.