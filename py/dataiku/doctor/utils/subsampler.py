...
class Subsampler(object):
    ...
    def balanced_subsampling(self,):
        ...
        for val, count in value_counts.items():
            if count > target_sample_size_per_class:
                value_selected_rows = pd.Index(random.sample(target[target == val].index, target_sample_size_per_class))
    ...
    def cluster_sampling(self):
        ...
        cluster_list = random.sample(self.values, k)
